...
**LogScope** is a self-hosted structured log collection and query service for web applications. Users clone the repository, run it locally, and use it for development debugging. It aggregates logs from backend (Node/Express) and frontend (browser) into a unified queryable store.

**Current Status: v1.0.0 Release Ready**
- All 8 phases complete (see `docs/progress.md`)
- All tests passing (backend 7/7, frontend 10/10)
- Documentation complete (README, GETTING_STARTED, DOCKER, DEPLOYMENT, ENVIRONMENT)
...
| Frontend | React 19, Vite 7, Bootstrap 5, TypeScript 5.9 |
...
server/                  # Express backend
  src/
    ...
    types/              # Data models
    storage/            # File I/O, query index, starred storage
...
web/                    # React frontend
  src/
    ...
    components/         # FilterPanel, LogTable, StatsPanel
```
...
**Note:** File watching is enabled. No restart needed for code changes.
...
## LogScope-Specific Conventions

### Code Patterns
- Use `import type` for type-only imports
- Types in `src/types/`, business logic in feature folders
- Interfaces prefixed with `I` (e.g., `IFileStorage`, `IQueryIndex`, `IStarredStorage`)
- Factory functions for creating storage instances (e.g., `createFileStorage()`)
...
### API Patterns
- All responses use `{ success: true/false, data?, error?, errorCode? }`
- Rate limiting: 10,000 req/min (configurable via env)
- Pagination: `limit` (max 1000) + `offset` parameters
- WebSocket path: `/ws` with subscribe/unsubscribe messages

### Storage Patterns
- File-based JSON storage in `logs/` directory
- Starred logs stored in `logs/starred.json`
- In-memory query index rebuilt on startup
- Auto-cleanup respects starred (pinned) logs
...
| `CLEANUP_INTERVAL_MS` | 10000 | Auto-cleanup check interval |
| `LOG_MAX_TOTAL` | 500 | Max logs before capacity cleanup |
| `LOG_MAX_AGE_MS` | 3600000 | Max log age before time cleanup |
| `LOG_DELETE_COUNT` | 100 | Logs to delete per cleanup cycle |

### Testing
- Backend: Vitest with 7 tests (validation, storage, routes, filters)
- Frontend: Vitest with 10 tests (components, API service)
- Run backend tests: `cd server && npm test`
- Run frontend tests: `cd web && npm test`

### Development Notes
- File watching enabled — no restart needed for code changes
- Backend uses nodemon with polling
- Frontend uses Vite dev server with polling
- Both servers should always be running during development
